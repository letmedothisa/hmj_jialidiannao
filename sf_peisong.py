...
def get_eleorder():
    ...
    sql = "SELECT * FROM order_2 WHERE creat_time > '%s' and dingdan_status!='%s' and (shop_id='%s' or shop_id='%s') and peisongy ='%s' and ps_record is NULL and deliverTime='%s'" % (now_time, -4, '1848879', '155852887', None,'0')
...
def get_status():
    ...
    sql = "SELECT * FROM ps_record WHERE sf_fadantime is not NULL and cancel_time is NULL and sf_donetime is NULL and creat_time>'%s'" % (now_time)
        ...
        for data in data_sql:
            ...
            if error_code==0:
                ...
                if data[6]==None and (sf_status==10 or sf_status==12):
                    ...
                    sql = "update ps_record set sf_taketime = '%s',rider_name='%s',rider_phone='%s',sf_status='%s' where orderid='%s'" % (datetime.datetime.now(),rider_name,rider_phone,sf_status,data[2])
