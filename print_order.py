...
while 1:
    ...
    sql_time = datetime.datetime.now() + datetime.timedelta(hours=-2)

    sql = "SELECT * FROM order_2 where order_2.creat_time > '%s'" % (sql_time)

    data_dingdan = []
    while 1:
        try:
            data_dingdan = mysql_tengxun.mysql_getdata(sql)
            break
        except:
            time.sleep(1)
    ...
    if data_dingdan:
        ...
        for dingdan in data_dingdan:

            if dingdan[16] == None and dingdan[4]=='美团':

                shangpin = get_dingdan(dingdan[9])
                ...
                for i in range(len(ddd)):
                    ...
                    li_1 = []
                    res_food = jsonpath.jsonpath(ddd[i], '$..food_name')

                    res_quantity = jsonpath.jsonpath(ddd[i], '$..quantity')

                    sql = "insert into order_detail set creat_time='%s',platform='%s',shop_id='%s',shop_name='%s',order_number='%s',order_id='%s',details='%s',details_quantity='%s'" % (
                        datetime.datetime.now(),dingdan[4],dingdan[10],dingdan[5],dingdan[6],dingdan[9],res_food[0],res_quantity[0])
                    while 1:
                        try:
                    ...
                    sql = "update order_2 set get_detail = '%s' where order_id = '%s'" % (1,dingdan[9])
            ...
            elif dingdan[16] == None and dingdan[4]=='饿了么':

                shangpin = get_elemedingdan(dingdan[9])


                shangpin = json.loads(shangpin)


                ddd = shangpin['result']['groups']

                li = []
                tupian_changdu = 0
                for i in range(len(ddd) - 1):

                    item_name = jsonpath.jsonpath(ddd[0], '$..name')
                    item_shul = jsonpath.jsonpath(ddd[0], '$..quantity')

                    k = len(item_name)
                    j = 0
                    while j < k:
                        if '口味' in item_name[j] or '篮子' in item_name[j] or '规格' in item_name[j]:
                            del item_name[j]
                            k -= 1
                            j -= 1
                        j += 1
                    ...
                    for j in range(len(item_name)):
                        ...
                        sql = "insert into order_detail set creat_time='%s',platform='%s',shop_id='%s',shop_name='%s',order_number='%s',order_id='%s',details='%s',details_quantity='%s'" % (
                            datetime.datetime.now(), dingdan[4], dingdan[10], dingdan[5], dingdan[6], dingdan[9],item_name[0], item_shul[0])
                        while 1:
                            try:
                                mysql_tengxun.mysql_db(sql)
                                print('insert_one_data')
